...
<div class="box">
	...
	<c:choose>
		...
		<c:otherwise>
			<form action="action/event/batchDelete">
				<!-- 批量删除 -->
				...
				<table class="dataTable">
					...
					<tr>
						<td>
							<input type="checkbox" name="deleteId" value="${event.id }" class="common-checkbox"/>
						</td>
						<td>
						...
						<td><fmt:formatDate value="${event.modifytime }" type="both"/></td>
						<td>
							<a href="action/event/viewModify?id=${event.id }" class="modify">编辑</a>
						</td>
					</tr>
					</c:forEach>
				</table>
				...
				<div id="pageBar">
					<div id="barL" class="l">
						...
						<c:choose>
							...
							<a href="action/event/filter?by=${by }&order=${order }&page=1&countPerPage=${pager.countPerPage}"
						...
						<c:choose>
							<c:when test="${pager.currentPage > 1 }">
								<a href="action/event/filter?by=${by }&order=${order }&page=${pager.currentPage - 1 }&countPerPage=${pager.countPerPage}"
							...
							<c:when
								...
								<c:forEach var="p" begin="1"
									...
									<c:choose>
										...
										<c:otherwise>
											<a href="action/event/filter?by=${by }&order=${order }&page=${p }&countPerPage=${pager.countPerPage}"
						...
						<c:choose>
							<c:when test="${pager.currentPage < pager.totalPage }">
								<a href="action/event/filter?by=${by }&order=${order }&page=${pager.totalPage}&countPerPage=${pager.countPerPage}"
					...
					<div id="barR" class="r">
						...
						<input type="button" value="GO" class="go-bt" id="action/event/filter?by=${by }&order=${order }&page=${pager.currentPage}${queryCondition}"/>
					</div>
					<div class="clear"></div>
				</div>
				<!-- 分页 end -->
