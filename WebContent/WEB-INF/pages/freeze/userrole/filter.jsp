		...
		<c:otherwise>
			<form action="action/userrole/batchDelete">
				...
				<table class="dataTable">
					...
					<c:forEach var="userrole" items="${dataList }">
					<tr>
						<td>
							<input type="checkbox" name="deleteId" value="${userrole.id }" class="common-checkbox"/>
						</td>
						<td>${userrole.userID }</td>
						<td>${userrole.roleID }</td>
						<td>
							<a href="action/userrole/viewModify?id=${userrole.id }" class="modify">编辑</a>
						</td>
					</tr>
					</c:forEach>
				</table>
